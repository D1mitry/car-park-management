...
public static class DomainExceptionExtensions
{
    ...
    public static DomainValidationException BecauseTimeOutEmpty(this Parking record)
        => new($"It is impossible to leave the parking space due to missing the departure time (VehicleReg: {record.VehicleReg}, VehicleType: {record.VehicleType}).") { Source = nameof(record) };
    ...
    public static DomainValidationException BecauseBasicChargeMissing(this Parking record)
        => new($"It is impossible to get the basic charge for this type of vehicle (VehicleReg: {record.VehicleReg}, VehicleType: {record.VehicleType}).") { Source = nameof(record) };

    internal static DomainValidationException BecauseEmptyRegistrationNumber(this string registration)
        => new($"Registration number cannot be empty.") { Source = nameof(registration) };
    ...
    internal static DomainConflictException BecauseVehicleAlreadyLeft(this Parking record)
        => new($"Vehicle with registration number {record.VehicleReg} is already left the parking space {record.ParkingSpaceNumber} as {record.TimeOut}.") { Source = nameof(record) };
}
